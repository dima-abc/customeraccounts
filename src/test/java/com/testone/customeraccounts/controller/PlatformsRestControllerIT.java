...
public class PlatformsRestControllerIT {
    ...
    void findPlatformReturnsPlatformList() throws Exception {
        var requestBuilder = MockMvcRequestBuilders.get("/customer-api/platforms")
                .param("p-name", "email1");
    ...
    void createPlatformRequestIsValidReturnsNewPlatform() throws Exception {
        var requestBuilder = MockMvcRequestBuilders.post("/customer-api/platforms")
        ...
        this.mockMvc.perform(requestBuilder)
                ...
                .andExpectAll(
                        status().isCreated(),
                        header().string(HttpHeaders.LOCATION, "http://localhost/customer-api/platforms/5"),
                        content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON),
                        content().json("""
                                {
                                    "id": 5,
    ...
    void createPlatformRequestIsInvalidReturnsProblemDetail() throws Exception {
        ...
        this.mockMvc.perform(requestBuilder)
                ...
                .andExpectAll(
                        ...
                        content().json("""
                                {
                                    "errors":["Поле platformName не должно быть пустым."]
                                }"""));
    }
}
